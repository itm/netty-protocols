...
public class DiscardMessagesHandlerFactory implements HandlerFactory {
	...
	public List<Tuple<String, ChannelHandler>> create(final String instanceName,
		...
		final boolean discardUpstream = HandlerFactoryPropertiesHelper.getFirstValueOf(properties, KEY_DISCARD_UPSTREAM, true);
		final boolean discardDownstream = HandlerFactoryPropertiesHelper.getFirstValueOf(properties, KEY_DISCARD_DOWNSTREAM, true);;

		return newArrayList(new Tuple<String, ChannelHandler>(instanceName, new DiscardMessagesHandler(discardUpstream, discardDownstream)));
	}
